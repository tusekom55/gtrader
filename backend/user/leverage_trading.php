...
function getOpenPositions($user_id) {
    ...
    $stmt = $conn->prepare("
        SELECT lp.*, c.current_price as market_price,
               (CASE 
                   WHEN lp.position_type = 'long' THEN 
                       (c.current_price - lp.entry_price) * lp.position_size
                   ELSE 
                       (lp.entry_price - c.current_price) * lp.position_size
               END) as unrealized_pnl,
               (CASE 
                   WHEN lp.position_type = 'long' THEN 
                       ((c.current_price - lp.entry_price) / lp.entry_price) * 100 * lp.leverage_ratio
                   ELSE 
